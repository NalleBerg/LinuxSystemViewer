...
private slots:
    ...
    void onSearchResultClicked(int row, int column)
    {
        ...
        if (targetTable && resultData.rowIndex >= 0 && resultData.rowIndex < targetTable->rowCount()) {
            // Make sure the row is visible (in case search filtering is active)
            targetTable->setRowHidden(resultData.rowIndex, false);
            ...
            // Highlight all matching text in the row
            for (int col = 0; col < targetTable->columnCount(); ++col) {
                QTableWidgetItem *targetItem = targetTable->item(resultData.rowIndex, col);
                if (targetItem && targetItem->text().contains(resultData.searchTerm, Qt::CaseInsensitive)) {
                    highlightMatchedText(targetTable, resultData.rowIndex, col, resultData.searchTerm);
                }
            }
            
            // Update status
            statusLabel->setText(QString("Navigated to match: %1").arg(resultData.searchTerm));
            QTimer::singleShot(2000, [this]() { statusLabel->setText("Ready"); });
        }
    }
    ...
    void highlightMatchedText(QTableWidget *table, int row, int col, const QString &searchTerm)
    {
        ...
        if (item && item->text().contains(searchTerm, Qt::CaseInsensitive)) {
            // Highlight the matched cell with a bright yellow background
            item->setBackground(QColor(255, 255, 0, 180)); // Bright yellow with some transparency
            item->setForeground(QColor(0, 0, 0)); // Ensure black text for contrast
        }
    }
    ...
    void onSearchTextChanged(const QString &text)
    {
        ...
        if (text.isEmpty() || text.length() < 2) { // Require at least 2 characters for search
        ...
        for (int tabIndex = 0; tabIndex < tables.size(); ++tabIndex) {
            ...
            for (int row = 0; row < table->rowCount(); ++row) {
                ...
                for (int col = 0; col < table->columnCount(); ++col) {
                    QTableWidgetItem *item = table->item(row, col);
                    if (item && item->text().toLower().contains(searchTextLower)) {
                        hasMatch = true;
                        
                        if (matchedProperty.isEmpty()) {
                            // Get the property name from the first column or header
                            if (col == 0) {
                                matchedProperty = item->text();
                            } else {
                                matchedProperty = (table->horizontalHeaderItem(col) ? 
                ...
                if (hasMatch) {
                    // Collect row details (first 3 columns for context)
                    QStringList rowData;
                    ...
                    // Create items for the result with explicit styling
                    ...
                    // Ensure text is visible (black on white)
                    QColor textColor = QColor(0, 0, 0);  // Black text
                    QColor bgColor = QColor(255, 255, 255);  // White background
                    
                    tabItem->setForeground(QBrush(textColor));
                    propertyItem->setForeground(QBrush(textColor));
                    valueItem->setForeground(QBrush(textColor));
                    detailsItem->setForeground(QBrush(textColor));
                    
                    tabItem->setBackground(QBrush(bgColor));
                    propertyItem->setBackground(QBrush(bgColor));
                    valueItem->setBackground(QBrush(bgColor));
                    detailsItem->setBackground(QBrush(bgColor));
                    ...
                    QString tooltip = QString("Click to navigate to %1 tab, row %2").arg(tabName).arg(row + 1);
                    tabItem->setToolTip(tooltip);
                    propertyItem->setToolTip(tooltip);
                    valueItem->setToolTip(tooltip);
                    detailsItem->setToolTip(tooltip);
        ...
        if (totalResults > 0) {
            ...
            if (!text.isEmpty() && text.length() >= 2) {
                statusLabel->setText(QString("No results found for: %1").arg(text));
                QTimer::singleShot(3000, [this]() { statusLabel->setText("Ready"); });
            }
        }
    }
...
private:
    ...
    void createHardwareTables()
    {
        ...
        storageTable = createTable({"Device", "Mount Point", "Size", "Used", "Available", "Use%", "Filesystem", "Type", "Model"});
        ...
        // Fix search results table styling to prevent black on black text
        searchResultsTable->setStyleSheet(
            "QTableWidget { "
            "   font-size: 8pt; "
    ...
    void connectSignals()
    {
        ...
        // Setup storage auto-refresh timer
        storageRefreshTimer = new QTimer(this);
        connect(storageRefreshTimer, &QTimer::timeout, this, &LinInfoGUI::refreshStorageTab);
        storageRefreshTimer->start(1000); // Refresh every second
        
        // Setup summary auto-refresh timer for live values
        summaryRefreshTimer = new QTimer(this);
        connect(summaryRefreshTimer, &QTimer::timeout, this, &LinInfoGUI::refreshSummaryTab);
        summaryRefreshTimer->start(1000); // Refresh every second
    }
    ...
    void loadLiveStorageInformation()
    {
    ...
    void loadSummaryInformation()
    {
        ...
        // Add active network connection information
        QMap<QString, NetworkInfo> networkInterfaces = getNetworkInterfaces();
        QString defaultGateway = getDefaultGateway();
        
        // Find the primary active interface (the one with a gateway)
        QString primaryInterface;
        QString primaryIP;
        for (auto it = networkInterfaces.begin(); it != networkInterfaces.end(); ++it) {
            const NetworkInfo &info = it.value();
            if (info.isActive && !info.ipv4Address.isEmpty() && info.ipv4Address != "127.0.0.1") {
                primaryInterface = it.key();
                primaryIP = info.ipv4Address;
                break; // Take the first active non-localhost interface
            }
        }
        
        if (!primaryInterface.isEmpty()) {
            QString connectionInfo = QString("%1 (%2)").arg(primaryInterface).arg(primaryIP);
            addPropertyToTable(summaryTable, "Network Connection", connectionInfo);
            
            if (!defaultGateway.isEmpty()) {
                addPropertyToTable(summaryTable, "Default Gateway", defaultGateway);
            }
        } else {
            addPropertyToTable(summaryTable, "Network Connection", "No active connection");
        }
        
        // Add live storage information
        addLiveStorageToSummary();
    }
    
    void addLiveStorageToSummary()
    {
        // Get the root filesystem info (most important for summary)
        QStorageInfo rootStorage = QStorageInfo::root();
        if (rootStorage.isValid()) {
            qint64 totalBytes = rootStorage.bytesTotal();
            qint64 availableBytes = rootStorage.bytesAvailable(); 
            qint64 usedBytes = totalBytes - availableBytes;
            ...
            // Calculate usage percentage
            double usagePercent = totalBytes > 0 ? (double(usedBytes) / totalBytes) * 100.0 : 0.0;
            QString usageInfo = QString("%1 / %2 (%3%)")
                               .arg(usedSize)
                               .arg(totalSize) 
                               .arg(QString::number(usagePercent, 'f', 1));
            
            addPropertyToTable(summaryTable, "Root Storage", usageInfo);
            addPropertyToTable(summaryTable, "Available Space", availableSize);
        }
        
        // Count total number of mounted storage devices
        QList<QStorageInfo> storageList = QStorageInfo::mountedVolumes();
        ...
        for (const QStorageInfo &storage : storageList) {
            ...
            validStorageCount++;
            totalSystemStorage += storage.bytesTotal();
            totalSystemUsed += (storage.bytesTotal() - storage.bytesAvailable());
        }
        
        if (validStorageCount > 1) {
            addPropertyToTable(summaryTable, "Storage Devices", QString::number(validStorageCount));
            
            QString totalSystemInfo = QString("%1 / %2")
                                     .arg(formatSize(totalSystemUsed))
                                     .arg(formatSize(totalSystemStorage));
            addPropertyToTable(summaryTable, "Total System Storage", totalSystemInfo);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    // Set application icon globally for file handlers from embedded resource
    QIcon appIcon(":LinInfoGUI.png");
    if (!appIcon.isNull()) {
        app.setWindowIcon(appIcon);
        qDebug() << "Global icon set from embedded resource";
    } else {
        qDebug() << "Warning: Could not load embedded icon resource";
    }
